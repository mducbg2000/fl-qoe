...
def read_chunk(id: int) -> tuple[pd.DataFrame, pd.Series]:
    X_raw, y = read_raw_dataset(
        "datasets/pokemon_users/chunk{}.csv".format(id), "id", "MOS", ["user_id"])
...
if __name__ == "__main__":
    ...
    global_weights = fed_avg(
        [(result[0].get_weights(), result[1]) for result in train_results])

    global_model.set_weights(global_weights)

    X_test, y_test = read_chunk(181)

    local_mse = [compute_loss(model, X_test, y_test)
                 for (model, _) in train_results]
